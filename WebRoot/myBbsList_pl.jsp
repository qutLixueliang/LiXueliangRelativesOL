       ...
       <table class="layui-table" lay-filter="mylist" lay-size="lg" >
		<thead>
			<tr>
				...
				<th lay-data="{field:'time',align:'center', minWidth:170}">评论内容</th>
			    <th lay-data="{field:'www',align:'center',minWidth:260}">评论时间</th>
               <th lay-data="{field:'sj',align:'center',minWidth:260}">板块</th>
		...
		<tbody style="height: 250px;">
		 ...
		 <tr>
				...
				<a href="showNewx_lt.do?id=${n.id }">${n.name }</a></c:if>
				</c:forEach></td>
				
				<td>${u.note }</td>
	          	<td>${u.pubtime }</td>
	          	<td>${u.gtype }</td>
			  <td><a class="layui-btn layui-btn-danger layui-btn-xs color2"  onclick="return del(1,1,1)" href="deleteBbs_pl.do?id=${u.id }" >删除</a>
			  </td>
			</tr>
        </c:forEach> 
		</tbody>  

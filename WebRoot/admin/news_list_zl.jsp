...
<div class="wangid_conbox">
	...
	<!-- 筛选 --> 
	<form action="newsListSearch_zl.do" method="post" >
	<div class="shuaix">
		...
		<div class="right">
			<input type="text" name="name" placeholder="请输入指南的名称">
	...
	<table class="layui-table" lay-filter="mylist" lay-size="lg">
		<thead>
			<tr>
				...
				<th lay-data="{field:'yx', align:'center',width:260}">名称</th>
				<%--<th lay-data="{field:'time',align:'center', minWidth:170}">状态</th>--%>
		...
		<tbody>
		 ...
		 <tr>
				...
				<td>${num.count }</td>
				<td>${u.name }</td>
			<%--	<td><c:forEach items="${klist }" var="t">
			  ...
			  <td>
                  <c:if test="${sessionScope.auser.utype=='管理员'}">
                      <c:if test="${u.status=='待审核'}">
                          <a class="layui-btn layui-btn-xs"   href="news_tongGuo.do?id=${u.id }" >通过</a>
                          <a class="layui-btn layui-btn-danger layui-btn-xs color2"   href="news_buTongGuo.do?id=${u.id }" >不通过</a>
                      </c:if>
                  </c:if>
                  <a class="layui-btn layui-btn-xs"   href="doUpdateNews_zl.do?id=${u.id }" >修改</a>
				<a class="layui-btn layui-btn-danger layui-btn-xs color2"  onclick="return del(1,1,1)" href="deleteNews_zl.do?id=${u.id }" >删除</a>
			  </td>
			</tr>
        </c:forEach> 
		</tbody>  
...
<script type="text/javascript">
    ...
    function add(){
        ...
        layer.open({
            ...
            // time: 2000, //2秒后自动关闭
            anim: 5,//动画
            content: ['doAddNews_zl.do', 'yes'], //iframe的url，no代表不显示滚动条
        }); 

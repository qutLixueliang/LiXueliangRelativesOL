...
<div class="wangid_conbox">
	...
	<div class="shuaix">
		...
		<div class="right">
			<input type="text" name="name" placeholder="请输入寻亲的名称">
	...
	<table class="layui-table" lay-filter="mylist" lay-size="lg">
		<thead>
			<tr>
				...
				<th lay-data="{field:'fl',align:'center', minWidth:60}">分类</th>
				<th lay-data="{field:'www',align:'center',minWidth:60}">发布人</th>
				<th lay-data="{field:'zy',align:'center',minWidth:60}">志愿者</th>
				<th lay-data="{field:'xz',align:'center',minWidth:60}">文件下载</th>
		...
		<tbody>
		 ...
		 <tr>
             ...
             <td><c:forEach items="${ualist }" var="t">
                 <c:if test="${t.id==u.zyid }">${t.tname }</c:if>
             </c:forEach></td>
              <td> <c:if test="${u.upload!=null }">
          <a title="下载文件"  href="#" onclick="down1('./upload/${u.upload }')" class="btn btn-xs btn-info" style="color:#f00;"><i class="icon-edit bigger-120">文件下载</i></a>
          </c:if>
           <c:if test="${u.upload==null }">
          暂无文件
          </c:if></td>
			  ...
			  <td>
                  ...
                  <c:if test="${u.status=='待审核'}">
                      <a class="layui-btn layui-btn-xs"   href="goods_tongGuo.do?id=${u.id }" >通过</a>
                      <a class="layui-btn layui-btn-danger layui-btn-xs color2"  onclick="return del2(1,1,1)" href="goods_buTongGuo.do?id=${u.id }" >不通过</a>
                  </c:if>
                  </c:if>
                  <a class="layui-btn layui-btn-xs"   href="doUpdateGoods.do?id=${u.id }" >修改</a>
